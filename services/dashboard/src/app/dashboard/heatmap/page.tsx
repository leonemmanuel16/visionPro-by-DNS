...
import { Flame, User, Calendar, Camera, Filter } from "lucide-react";
...
function generateHeatData(seed: number) {
  ...
  for (let y = 0; y < 12; y++) {
    const row: number[] = [];
    for (let x = 0; x < 16; x++) {
      // Create hotspots near certain areas
...
const DEMO_CAMERAS_HEAT = [
  { id: "cam-001", name: "Entrada Principal", location: "Lobby" },
  { id: "cam-002", name: "Estacionamiento Norte", location: "Parking Lot" },
  { id: "cam-003", name: "Oficina Servidores", location: "Server Room" },
  { id: "cam-005", name: "Almacén", location: "Warehouse" },
  { id: "cam-006", name: "Recepción", location: "Reception" },
];
...
export default function HeatmapPage() {
  ...
  const cameraInfo = DEMO_CAMERAS_HEAT.find((c) => c.id === selectedCamera);
              ...
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base flex items-center gap-2">
                    ...
                    {cameraInfo?.name} — {cameraInfo?.location}
                  </CardTitle>
                  {selectedPerson !== "all" && (
